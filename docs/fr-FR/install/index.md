...
read_when:
  - Vous avez besoin d'une méthode d'installation autre que le démarrage rapide du Guide de démarrage
...
Vous avez déjà suivi [Démarrage](/start/getting-started) ? Vous êtes prêt — cette page est pour les méthodes d'installation alternatives, les instructions spécifiques à la plateforme et la maintenance.

## Configuration requise

- **[Node 22+](/install/node)** (le [script d'installation](#méthodes-dinstallation) l'installera s'il est manquant)
...
<AccordionGroup>
  <Accordion title="Script d'installation" icon="rocket" defaultOpen>
    ...
    Pour tous les indicateurs, variables d'environnement et options CI/automatisation, voir [Détails du programme d'installation](/install/installer).
    ...
    <Tabs>
      <Tab title="npm">
        ...
        <Accordion title="Erreurs de build sharp ?">
          Si vous avez libvips installé globalement (courant sur macOS via Homebrew) et que `sharp` échoue, forcez les binaires pré-compilés :

          ```bash
          SHARP_IGNORE_GLOBAL_LIBVIPS=1 npm install -g openclaw@latest
          ```

          Si vous voyez `sharp: Please add node-gyp to your dependencies`, installez soit les outils de build (macOS : Xcode CLT + `npm install -g node-gyp`) ou utilisez la variable d'environnement ci-dessus.
        </Accordion>
      </Tab>
      <Tab title="pnpm">
        ```bash
        ...
        <Note>
        pnpm nécessite une approbation explicite pour les packages avec des scripts de build. Après que la première installation affiche l'avertissement "Ignored build scripts", exécutez `pnpm approve-builds -g` et sélectionnez les packages listés.
        </Note>
      </Tab>
    </Tabs>
  ...
  <Accordion title="Depuis les sources" icon="github">
    ...
    Pour des flux de travail de développement plus approfondis, voir [Configuration](/start/setup).
...
Voir [Variables d'environnement](/help/environment) pour la précédence et les détails complets.

## Dépannage : `openclaw` introuvable

<Accordion title="Diagnostic et correction de PATH">
  Diagnostic rapide :
