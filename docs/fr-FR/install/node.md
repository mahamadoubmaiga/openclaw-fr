...
OpenClaw nécessite **Node 22 ou plus récent**. Le [script d'installation](/install#méthodes-dinstallation) détectera et installera Node automatiquement — cette page est pour quand vous voulez configurer Node vous-même et vous assurer que tout est correctement configuré (versions, PATH, installations globales).
...
Si cela affiche `v22.x.x` ou supérieur, vous êtes prêt. Si Node n'est pas installé ou que la version est trop ancienne, choisissez une méthode d'installation ci-dessous.
...
Après l'installation de Node, vérifiez que tout fonctionne :

```bash
node -v        # devrait afficher v22.x.x ou supérieur
npm -v         # devrait afficher la version de npm
npm prefix -g  # devrait afficher le répertoire d'installation global
```
...
1. Installez OpenClaw : [Installation](/install)
2. Exécutez l'assistant d'intégration : [Démarrage](/start/getting-started)
3. Configurez votre première session de chat
